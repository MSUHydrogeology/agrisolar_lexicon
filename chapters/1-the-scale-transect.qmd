...
As projects grow, restoration changes instrument. All five [design levers](0-design-levers.qmd) remain available at every size, but not at the same strength, and the transect does something different to each of them.
...
Siting and management are the pair that cross, around A3–A4, and that crossing is what Figure 1.2 draws. Which element did the restorative work is the project's [restoration mode](0-design-levers.qmd#sec-lv-mode), the third tag the archetypes carry: **siting-led** where the work is done by choosing the ground, **structure-led** where the hardware does it, **management-led** where placement is fixed and the ground cover and grazing carry everything, and **contract-led** where it happens on paper and scale has no say at all.
