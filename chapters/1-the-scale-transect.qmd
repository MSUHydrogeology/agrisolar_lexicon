...
A transect works because it is not one variable. It is a **bundle of covarying attributes** — several features moving together along one gradient — so naming a zone tells you many things at once. (An ecological transect from shore to upland shifts soil, moisture, and vegetation all together in just this way.) The device is borrowed rather than invented: the cross-section of a landscape traces back to Humboldt, who drew shore, wetland, plain, and upland as one continuous gradient long before the idea was applied to human settlement. Using it on agrisolar land returns it to something like its origins.
...
**Calibration note.** Capacity bands are indicative for the US Midwest and should be re-calibrated regionally. The zone is defined by the attribute bundle, not the megawatts. Nor does every zone appear everywhere: a region with no transmission-adjacent land simply has no A5, which is a fact about that region and not a deficiency in it.
