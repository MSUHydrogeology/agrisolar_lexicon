...
![**Scale Transect Zones.** Calibration points on the gradient rather than boxes with walls: named places to stand, so that a project near a boundary can be read as being near a boundary. Each panel is drawn at its own scale, so what grows across the set is the share of the frame the array takes and what else is still in view, not the size of the frame. Capacity is how a project is filed and acreage is how a place experiences it, so where the two disagree the acres are the better guide. Building-mounted arrays are **A0 — Rooftop** and take no ground at all, which puts them at the origin of the gradient rather than on it.](../figures/fig_scale_transect.svg){#fig-scale-transect}
...
Building-mounted arrays are **A0 — Rooftop**: no ground to choose, manage, share, or spare. A5 has no ceiling; its common condition is placement fixed by [land assembly](terminology.qmd#term-land-assembly) and restoration carried by management (§-@sec-siting-latitude).
