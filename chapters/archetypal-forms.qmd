...
![**How an archetype emerges.** An energy landscape, where each basin is a setting of the six [design levers](design-levers.qmd) that holds together. Projects roll to the bottom of whichever basin they are near, so the deep ones fill and the shallow ones do not, and a setting that keeps filling is what this chapter calls an archetype. Nothing here is a design process: no project chose its basin, and the basins are the shape the [pressures](design-pressures.qmd) leave behind once they have finished arguing. [A region weights those pressures differently](design-pressures.qmd#sec-regionality), which reshapes the landscape rather than moving projects around inside it. Retiring irrigated ground is the deepest basin in an arid region and nearly flat in a humid one; prairie strips are the reverse. **A basin that flattens is a form that stops being built.**](../figures/fig_archetypes_emerge.svg){#fig-archetypes-emerge}
