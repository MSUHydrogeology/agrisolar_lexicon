...
![**How an archetype emerges.** Design tension against the [design pressures](design-pressures.qmd): a position along the bottom is one way the nine can come out, and the height is how much conflict that resolution leaves standing. Where the pressures accommodate each other the curve drops, and projects settle into the low spots the way water finds a hollow. A low spot that keeps collecting projects is what this chapter calls an archetype, and the six worked through below are named on the curve with the code each carries. They run left to right by the lever the restorative work fell to, from ownership through structure and management to siting, which is the nearest thing the horizontal axis has to an order. Nothing here is a design process: no project chose its basin, and the basins are the shape the pressures leave behind once they have finished arguing. The landscape is also not the same everywhere, because [a region weights the pressures differently](design-pressures.qmd#sec-regionality). The dashed profile is an arid region, where retiring irrigated ground becomes much the deepest basin on the curve, shade is worth enough to deepen the vegetable basin, and the prairie-strip basin flattens almost away. **A basin that flattens is a form that stops being built.**](../figures/fig_archetypes_emerge.svg){#fig-archetypes-emerge}
