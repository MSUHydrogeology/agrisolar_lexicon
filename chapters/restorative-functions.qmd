...
Nothing about the land separates them. Where a project sits (§-@sec-transect) and what happened to the farming on it (§-@sec-siting-axis) set the outer limits of what is possible there, but what the ground ends up doing is settled by how the design resolved its competing pressures (§-@sec-pressures-ch), and above all by whether anyone was still pushing for restoration when the cheaper option was on the table.

[*Restorative*](terminology.qmd#term-restorative) is therefore not something a piece of ground is. It is something a design produces, and on most sites it can be produced or withheld. **Restorative functions** name the five things a design can be aimed at.

- **Water** (§-@sec-fn-hydrologic) — what soaks in, what runs off, and what the runoff carries with it
- **Soil and carbon** (§-@sec-fn-soil) — what builds back up underneath
- **Ecology** (§-@sec-fn-biodiversity) — what can live there, and what can move through
- **Agriculture** (§-@sec-fn-agricultural) — what farming gets from it, here and on the fields around it
- **Community and economy** (§-@sec-fn-community) — what the people hosting it actually get

![Five functions, which lever carries each one, and which route its benefit travels. The route matters more than the distance: every function here reaches beyond the fence along some route, so the useful question is which one, since that decides who can check the claim and who receives it. Carbon is the exception the table cannot draw, leaving by air to everywhere and no one in particular. Land sparing is missing for a different reason: its benefit is ground elsewhere that stayed as it was, so nothing travels at all (§-@sec-benefit-range).](../figures/fig_functions_overview.svg){#fig-functions-overview}
...
**A function is not a position on the [siting axis](terminology.qmd#term-siting-axis).** A project holds one position, which records what happened to agricultural production on its ground. It can deliver several functions at once, and the same function turns up at many positions: pollinator habitat is reachable under grazing (S2), on ordinary converted ground (S3), and on spared or retired ground (S4–S5) alike. Position narrows the set at one end only, since the functions that need an undisturbed [ground layer](terminology.qmd#term-ground-layer) are out of reach under the annual tillage of full sharing (S1). The three registers stay distinct: a [pressure](terminology.qmd#term-design-pressures) is a force pushing on the design, a [lever](terminology.qmd#term-design-levers) is something the design does about it, and a function is the result the two produce. Treat the five as a palette rather than a scorecard, since no project delivers all of them and some work against each other.

**Two accounts, both needed.** The [siting axis](restoration-modes.qmd#sec-harm-benefit) is largely a ledger of harm reduced, since every position it records answers a question about displacement. These functions are where benefit gets promoted. The water function runs both ways at once and shows what the difference costs, because **avoided loading** needs the ground that was leaching while **interception** needs the ground below it, so one objective sends a designer to opposite parts of the same farm. The two are checked differently, too: a benefit can be gone to and looked at, while an avoided harm has to be argued against what would otherwise have happened. Neither ranks above the other, and on ground that was doing real damage, ending it is the larger result.
...
Construction cuts the other way and can cut deep. Grading removes the shallow horizon holding most of the carbon, trenching and traffic compact what is left, and compacted ground restricts the roots that would rebuild it, so a site can begin its thirty years in debt rather than at zero. How large that debt is depends on how much earth anyone had to move, which makes a racking choice a soil decision taken years before anybody picks a seed mix: a mounting system that tolerates slope leaves the ground roughly as it found it, and one that needs a level plane means mass grading. Establishment method, grazing regime, and [stocking density](terminology.qmd#term-stocking-density) then carry the accumulation, which is slow and cumulative and therefore hostage to a sustained operating regime.

This is the one function delivered almost entirely **in place**, since soil stays where it is, which also makes it the one most fully captured by whoever holds the ground. Carbon is the exception and the limiting case: it leaves by air, benefiting everywhere and no one in particular, which is why it needs a market or a policy behind it before it is worth anything to the person holding the land.

::: {.evidence}
**Evidence status.** Early gains are **verified** for [ecovoltaic](terminology.qmd#term-ecovoltaics) grassland restoration [@krasner2025], long-run accumulation is thinly measured [@carvalho2024], and the baseline sets the sign, since perennialization improves a tilled baseline while converting existing grassland creates a debt [@merheb2025]. The size of the construction debt is the least measured part of the ledger and among the likeliest to decide that sign, because published gains come from sites whose earthworks are not described.
:::
...
## Agriculture {#sec-fn-agricultural}

**Not the siting axis under another name.** The axis records whether farming continues on the ground the array occupies. This asks what the array does *for* farming, and the two answers come apart: a project with production stopped inside its own fence can still be why the field next door gets pollinated, and why the host farm is still in business.

Three routes. The ecological one runs along the same living network ecology does, carrying pollination into neighboring crops [@walston2024; @walston2018] and sheltering the predators and parasitoids that hold pest numbers down, which is the established case for perennial refuge in row-crop landscapes [@schulte2017] and makes an array a candidate piece of integrated pest management. The on-site one is forage and shelter under the panels, including shade for stock through the worst of the afternoon and for the people working there. The third runs on paper: lease revenue is what decides whether there is still a farm to serve, which is [coexistence](siting-axis.qmd#sec-positions) seen from the agricultural side.
...
::: {.evidence}
**Evidence status.** Pollinator movement into adjacent crops is **verified** [@walston2024] and the farmland inside foraging distance has been mapped [@walston2018]. Pest regulation is **inferred, not measured** at solar sites. The step that matters most to a farmer, a yield or input-cost change attributable to the array, is **not established anywhere** and should not be claimed. The enterprise route is the best evidenced of the three [@stid2025; @sturchio2025croplands].
:::
...
- **Tenant farmer** — frequently the party who *loses*. Nearly 40% of Central Valley agricultural land is rented, and the leading analysis of agrisolar economics there assumes returns reach landowning farmers, naming that as one of its own limitations [@stid2025]. This is the sharpest misalignment in the set, and the one a lease-income argument most often hides.
- **Neighboring residents** — bear viewshed, construction traffic, and glare, usually without a payment.
...
This function travels almost entirely on paper, along tenure, contract, and jurisdiction, which is why the list above does more work than any radius would. Part of it is genuinely local, since construction spend lands in the nearest town that can supply it, but the money that matters most moves along channels drawn for reasons that have nothing to do with distance.
...
2. **Who captures it.** A landowner captures in-place benefit and can be paid for it. Benefits travelling water or living things accrue to people holding no contract with the project, which is why they need a policy, a market, or a watershed authority behind them to amount to more than goodwill.
...
SCALE reads a project across **Sustainable Communities, Agriculture, Landscapes, and Energy**, each running from causing harm at the rim, through avoiding harm, to promoting benefit at the center (§-@sec-harm-benefit). Three of the four line up with a function: Communities with the community and economic function, Landscapes with water, soil, and ecology, and Agriculture with the agricultural function together with the siting axis. Energy is the exception, aligning with the design pressures rather than with anything this chapter produces. The reading is evaluative, but the ring where harm ends is a zero point rather than a pass mark, and how far past it a project ought to travel is left open (§-@sec-open-issues, issue 5).

![The SCALE footprint: a project's influence read across four domains (Sustainable Communities, Agriculture, Landscapes, Energy) and their sub-axes. Each runs from causing harm at the rim, through the dashed ring where a domain is left no worse than it was found, to promoting benefit at the center. The ring is a zero point and not a pass mark: it marks where the sign changes, not a standard a project is expected to clear, and how far past it to travel is still open (§-@sec-open-issues, issue 5). The reading is evaluative by design, which is what *restorative* commits the document to. The polygon shown is a worked example.](../figures/fig_scale_footprint.svg){#fig-scale-footprint}
