...
![Four functions, which lever carries each one, and which route its benefit travels. Two things the table cannot draw: carbon, which leaves by air, and land sparing, whose benefit is ground elsewhere that stayed as it was](../figures/fig_functions_overview.svg){#fig-functions-overview}

Read the table by route rather than by distance. Every function here reaches beyond the fence along some route, so the useful question is not how far a benefit goes but which way it travels, since that is what decides who can check the claim and who receives it. Carbon is the exception the drawing cannot hold, leaving by air to everywhere and no one in particular, and land sparing is missing for a different reason: nothing travels at all, because its benefit is ground that stayed as it was (§-@sec-benefit-range).
...
![The eight outcomes plotted together, four quadrants, one per function. Each spoke runs from causing harm at the centre, through the dashed ring where the outcome is left no worse than it was found, out to promoting benefit at the rim, so a larger shape is a better project.](../figures/fig_outcomes_footprint.svg){#fig-outcomes-footprint}
