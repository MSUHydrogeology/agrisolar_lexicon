...
cows and runs the milking parlour, and gives up generation at scale to do it. The same handful
...
What the document refuses is the other kind of prescription: the fixed values a code would
enforce, a required buffer width, a mandated seed mix, a single success metric. A shared
language that hardened into that would lock in today's practice and close down the
experimentation this field still needs. **The aim is argued; the dimensions are left to the
site.** [Regionality](chapters/7-regionality.qmd) is the reason — the pressures are the same
everywhere, but sun, water, crop, and utility decide how hard each one pushes, so the clearance
that serves the aim in one county defeats it in the next.
:::
